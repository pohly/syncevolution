                           ...
                           sysync::uInt16 aDebugFlags)
{
    ...
    if (err) {
        boost::throw_exception(BadSynthesisResult(std::string("cannot connect to engine '") + aEngineName + "'", static_cast<sysync::TSyErrorEnum>(err)));
    }
}
...
void SharedEngine::Disconnect()
{
    ...
    if (err) {
        boost::throw_exception(BadSynthesisResult("cannot disconnect engine", static_cast<sysync::TSyErrorEnum>(err)));
    }
}
...
void SharedEngine::InitEngineXML(const string &aConfigXML)
{
    ...
    if (err) {
        boost::throw_exception(BadSynthesisResult("Synthesis XML config parser error", static_cast<sysync::TSyErrorEnum>(err)));
    }
}
...
SharedSession SharedEngine::OpenSession(const string &aSessionID)
{
    ...
    if (err) {
        boost::throw_exception(BadSynthesisResult("opening session failed", static_cast<sysync::TSyErrorEnum>(err)));
    }
...
SharedKey SharedEngine::OpenSessionKey(SharedSession &aSessionH)
{
    ...
    if (err) {
        boost::throw_exception(BadSynthesisResult("opening session key failed", static_cast<sysync::TSyErrorEnum>(err)));
    }
                               ...
                               sysync::TEngineProgressInfo *aInfoP)
{
    sysync::TSyError err = m_engine->SessionStep(aSessionH.get(),
                                                 ...
                                                 aInfoP);
    if (err) {
        boost::throw_exception(BadSynthesisResult("proceeding with session failed", static_cast<sysync::TSyErrorEnum>(err)));
    }
}
...
SharedBuffer SharedEngine::GetSyncMLBuffer(const SharedSession &aSessionH, bool aForSend)
{
    ...
    sysync::TSyError err = m_engine->GetSyncMLBuffer(aSessionH.get(),
                                                     ...
                                                     buffer, bufSize);
    if (err) {
        boost::throw_exception(BadSynthesisResult("acquiring SyncML buffer failed", static_cast<sysync::TSyErrorEnum>(err)));
    }
...
void SharedEngine::WriteSyncMLBuffer(const SharedSession &aSessionH, const char *data, size_t len)
{
    ...
    if (err) {
        boost::throw_exception(BadSynthesisResult("writing SyncML buffer failed", static_cast<sysync::TSyErrorEnum>(err)));
    }
}
                                      ...
                                      bool noThrow)
{
    ...
    if (err) {
        ...
        if (err == sysync::DB_NoContent) {
            boost::throw_exception(NoSuchKey(what));
        } else {
            boost::throw_exception(BadSynthesisResult(what, static_cast<sysync::TSyErrorEnum>(err)));
        }
    }
    return SharedKey(key, FreeEngineItem(*this));
}
...
string SharedEngine::GetStrValue(const SharedKey &aKeyH, const string &aValName)
{
    ...
    if (err) {
        boost::throw_exception(BadSynthesisResult(string("error reading value ") + aValName, static_cast<sysync::TSyErrorEnum>(err)));
    }
    return s;
}
...
void SharedEngine::SetStrValue(const SharedKey &aKeyH, const string &aValName, const string &aValue)
{
    ...
    if (err) {
        boost::throw_exception(BadSynthesisResult(string("error writing value ") + aValName, static_cast<sysync::TSyErrorEnum>(err)));
    }
}
