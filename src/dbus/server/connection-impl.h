 ...
 * supported.
 */
class ConnectionImpl : public GDBusCXX::DBusObjectHelper
{
    StringMap m_peer;
    ...
    boost::shared_ptr<SessionImpl> m_session;
...
public:
     ...
     */
    static boost::shared_ptr<ConnectionImpl> createConnection(GMainLoop *loop,
...
private:
    ConnectionImpl(GMainLoop *loop,
                   bool &shutdownRequested,
                   const GDBusCXX::DBusConnectionPtr &conn,
                   const std::string &sessionID);
    boost::weak_ptr<ConnectionImpl> m_me;
