    ...
    if(m_session && !hasActiveSession()
       && boost::iequals(m_session->getConfigName(), configName)) {
        SE_LOG_DEBUG(NULL, NULL, "auto sync: removing queued session for %s during update",
                     configName.c_str());
        m_server.removeResource(boost::static_pointer_cast<Resource>(m_session), boost::bind(&AutoSyncManager::updateCb, this));
    }
}
...
void AutoSyncManager::startTaskCb(const boost::shared_ptr<Session> &resource)
{
    if (resource) {
        m_session = resource;
        m_session->setPriority(Resource::PRI_AUTOSYNC);
        m_session->addListener(this);
        m_server.addResource(m_session, boost::function<void()>(&NullCb));
...
void AutoSyncManager::startTask()
{
    ...
    if(hasTask() && !m_session) {
        ...
        Session::createSession(boost::bind(&AutoSyncManager::startTaskCb, this, _1),
                               m_server,
                               "",
                               m_activeTask->m_peer,
                               newSession);
...
void AutoSyncManager::prepareCb()
{
    m_session->syncAsync("",
                                 SessionCommon::SourceModes_t(),
                                 boost::function<void()>(&NullCb));
}
...
void AutoSyncManager::prepare()
{
    if(m_session && m_session->getActive()) {
        ...
        config[""] = stringMap;
        m_session->setConfigAsync(true, true, config, boost::bind(&AutoSyncManager::prepareCb, this));
    }
}
