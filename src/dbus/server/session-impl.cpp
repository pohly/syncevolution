...
void SessionImpl::setConfig(bool update, bool temporary,
...
void SessionImpl::setNamedConfig(bool &setConfig, const std::string &configName,
...
void SessionImpl::initServer(SharedBuffer data, const std::string &messageType)
{
    m_serverMode = true;
...
void SessionImpl::sync(const std::string &mode, const SessionCommon::SourceModes_t &source_modes)
{
    if (!m_active) {
        SE_THROW_EXCEPTION(InvalidCall, "session is not active, call not allowed at this time");
    }
    ...
    if (m_serverMode) {
        ...
        boost::shared_ptr<ConnectionImpl> c = m_connection.lock();
...
void SessionImpl::getStatus(std::string &status,
                            uint32_t &error,
...
void SessionImpl::getProgress(int32_t &progress,
...
void SessionImpl::fireStatus(bool flush)
{
    std::string status;
    uint32_t error;
    SessionCommon::SourceStatuses_t sources;
...
void SessionImpl::fireProgress(bool flush)
{
    int32_t progress;
    SessionCommon::SourceProgresses_t sources;
...
string SessionImpl::syncStatusToString(SessionCommon::SyncStatus state)
{
    switch(state) {
...
boost::shared_ptr<SessionImpl> SessionImpl::createSession(GMainLoop *loop,
...
{
    boost::shared_ptr<SessionImpl> me(new SessionImpl(loop, shutdownRequested, conn, config_name, session, flags));
    return me;
}

SessionImpl::SessionImpl(GMainLoop *loop,
                         bool &shutdownRequested,
                         const GDBusCXX::DBusConnectionPtr &conn,
                         const std::string &config_name,
                         const std::string &session,
    ...
    emitPasswordRequest(*this, "PasswordRequest")
{
    ...
    add(this, &SessionImpl::setNamedConfig, "SetNamedConfig");
    ...
    add(this, &SessionImpl::sync, "Sync");
    add(this, &SessionImpl::abort, "Abort");
    add(this, &SessionImpl::suspend, "Suspend");
    add(this, &SessionImpl::getStatus, "GetStatus");
    add(this, &SessionImpl::getProgress, "GetProgress");
    add(this, &SessionImpl::restore, "Restore");
    add(this, &SessionImpl::execute, "Execute");
    add(this, &SessionImpl::passwordResponse, "PasswordResponse");
    add(this, &SessionImpl::serverShutdown, "ServerShutdown");
    add(this, &SessionImpl::setActive, "SetActive");
    add(emitStatus);
    add(emitProgress);
...
void SessionImpl::serverShutdown()
{
    m_shutdownRequested = true;
    g_main_loop_quit(m_loop);
}

void SessionImpl::setActive(bool active)
{
    m_active = active;
    if (m_active) {
...
void SessionImpl::syncProgress(sysync::TProgressEventEnum type,
...
void SessionImpl::sourceProgress(sysync::TProgressEventEnum type,
                                 SyncSource &source,
                                 int32_t extra1, int32_t extra2, int32_t extra3)
{
...
void SessionImpl::run(LogRedirect &redirect)
{
    if (m_runOperation != OP_NULL) {
        try {
...
bool SessionImpl::setFilters(SyncConfig &config)
{
...
void SessionImpl::setStepInfo(bool isWaiting)
{
...
void SessionImpl::restore(const string &dir, bool before, const std::vector<std::string> &sources)
{
    if (!m_active) {
        SE_THROW_EXCEPTION(InvalidCall, "session is not active, call not allowed at this time");
    }
...
void SessionImpl::SyncStatusOwner::setStatus(SessionCommon::SyncStatus status)
{
...
string SessionImpl::runOpToString(RunOperation op)
{
    switch(op) {
    case OP_SYNC:
        return "sync";
...
void SessionImpl::execute(const vector<string> &args, const map<string, string> &vars)
{
    if (!m_active) {
        SE_THROW_EXCEPTION(InvalidCall, "session is not active, call not allowed at this time");
    }
...
void SessionImpl::passwordResponse(bool timed_out, const std::string &password)
{
...
string SessionImpl::askPassword(const string &passwordName,
                                const string &descr,
...
void SessionImpl::syncSuccessStart()
{
...
SessionListener* SessionImpl::addListener(SessionListener *listener)
{
