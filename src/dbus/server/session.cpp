...
void Session::sync2(const std::string &mode, const SessionCommon::SourceModes_t &sourceModes)
{
    ...
    m_helper->m_sync.start(params, boost::bind(&Session::dbusResultCb, this, "sync()", true, _1));
}
...
{
    try {
        SE_LOG_DEBUG(NULL, NULL, "%s helper call completed %s, %s",
                     operation.c_str(),
                     success ? "successfully" : "unsuccessfully",
                     error.empty() ? "without error" : error.c_str());
        if (error.empty()) {
            doneCb(success);
        } else {
...
void Session::restore2(const string &dir, bool before, const std::vector<std::string> &sources)
{
    ...
    m_helper->m_restore.start(m_configName, dir, before, sources,
                              boost::bind(&Session::dbusResultCb, this, "restore()", true, _1));
}
