...
void Session::sync(const std::string &mode, const SessionCommon::SourceModes_t &sourceModes)
{
    ...
    runOperationAsync(OP_SYNC,
                      // sync() is implemented synchronously, has no result pointer
                      boost::shared_ptr<GDBusCXX::Result0>(),
...
void Session::sync2(const std::string &mode, const SessionCommon::SourceModes_t &sourceModes)
{
    ...
    m_helper->m_sync.start(params, boost::bind(&Session::dbusResultCb, this, "sync()", _1));
}
...
void Session::dbusResultCb(const std::string &operation, const std::string &error) throw()
{
    try {
        SE_LOG_DEBUG(NULL, NULL, "%s helper call completed, %s",
                     operation.c_str(),
                     error.empty() ? "successfully" : error.c_str());
        if (error.empty()) {
            doneCb();
        } else {
            // Translate back into local exception, will be handled by
            // catch clause and (eventually) failureCb().
            if (boost::starts_with(error, "org.syncevolution.")) {
                // Strip prefix, give Exception::parse() a chance to
                // throw exactly the right internal exception. This is
                // important for cases like status exceptions, which
                // need to thrown here to set the right
                // Session::m_error code.
                const size_t pos = error.find(": ");
                if (pos != std::string::npos) {
                    const std::string exName(error.substr(0, pos));
                    const std::string msg(error.substr(pos + 2));
                    Exception::parse(msg);

                    // no recognized exception, rethrow as D-Bus exception
                    throw GDBusCXX::dbus_error(exName, msg);
                }
            }
            // generic fallback
...
void Session::failureCb() throw()
{
    try {
        if (m_status == SESSION_DONE) {
            ...
            uint32_t error;
            if (m_dbusResult) {
                // tell caller and record error (restore, command line, ...)
                error = dbusErrorCallback(m_dbusResult);
                m_dbusResult.reset();
            } else {
                try {
                    throw;
                } catch (...) {
                    // only record problem (sync)
...
void Session::doneCb() throw()
{
    try {
        ...
        if (m_dbusResult) {
            // finish pending operation (restore, command line, ...)
            m_dbusResult->done();
            m_dbusResult.reset();
        }
...
void Session::onQuit(int status) throw ()
{
    try {
        ...
        } else if (m_wasAborted  && WIFSIGNALED(status) && WTERMSIG(status) == SIGTERM) {
...
void Session::restore2(const string &dir, bool before, const std::vector<std::string> &sources)
{
    ...
    m_helper->m_restore.start(m_configName, dir, before, sources,
                              boost::bind(&Session::dbusResultCb, this, "restore()", _1));
}
...
void Session::execute2(const vector<string> &args, const map<string, string> &vars)
{
    ...
    m_helper->m_execute.start(args, vars,
                              boost::bind(&Session::dbusResultCb, this, "execute()", _1));
}
