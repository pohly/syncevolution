                ...
                private boost::noncopyable
{
    ...
    /**
     * Some method calls (Session.Restore, Session.Execute) return
     * success/failure of the entire operation. Session.Sync doesn't,
     * it returns as soon as the sync is started.
     *
     * So for Session.Restore and Session.Execute we use asynchronous
     * method implementations and remember how to signal the result
     * here. failureCb() and done() both check this and alert the
     * D-Bus client.
     */
    boost::shared_ptr<GDBusCXX::Result0> m_dbusResult;
...
private:
    ...
    void dbusResultCb(const std::string &operation, const std::string &error) throw();
