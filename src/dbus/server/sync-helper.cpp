...
namespace {
                   ...
                   const std::string &config, const std::string &sessionID, bool startSession)
    {
        connected = true;
        if (startSession) {
            dbusObjectHelper = SessionImpl::createSession(loop, shutdownRequested, conn, config, sessionID);
            ...
            dbusObjectHelper = ConnectionImpl::createConnection(loop, shutdownRequested, conn, sessionID);
            SE_LOG_DEBUG(NULL, NULL, "onConnect called in helper (path: %s interface: %s)",
                         dbusObjectHelper->getPath(), dbusObjectHelper->getInterface());
        }
...
int main(int argc, char **argv, char **envp)
{
    ...
    try {
        ...
        if (start_session) {
            boost::shared_ptr<SessionImpl> session = boost::dynamic_pointer_cast<SessionImpl>(dbusObjectHelper);
