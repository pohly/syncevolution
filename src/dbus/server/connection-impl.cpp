...
void ConnectionImpl::failed(const std::string &reason)
{
    if (m_failure.empty()) {
        m_failure = reason;
        if (m_session) {
...
void ConnectionImpl::wakeupSession()
{
    if (m_loop) {
...
void ConnectionImpl::process(const GDBusCXX::DBusArray<uint8_t> &message,
                             ...
                             bool must_authenticate)
{
    ...
    try {
        ...
        case SETUP: {
            ...
            m_state = PROCESSING;
            m_session = SessionImpl::createSession(m_loop,
                                                   m_shutdownRequested,
                                                   getConnection(),
                                                   config,
                                                   m_sessionID);
...
void ConnectionImpl::close(bool normal,
...
boost::shared_ptr<ConnectionImpl> ConnectionImpl::createConnection(GMainLoop *loop,
                                                                   bool &shutdownRequested,
                                                                   const GDBusCXX::DBusConnectionPtr &conn,
                                                                   const std::string &sessionID)
{
    boost::shared_ptr<ConnectionImpl> me(new ConnectionImpl(loop, shutdownRequested, conn, sessionID));
    me->m_me = me;
    return me;
}

ConnectionImpl::ConnectionImpl(GMainLoop *loop,
                               bool &shutdownRequested,
    ...
    m_state(SETUP)
{
    add(this, &ConnectionImpl::process, "Process");
    add(this, &ConnectionImpl::close, "Close");
    add(emitAbort);
    add(emitShutdown);
    add(emitReply);
...
bool ConnectionImpl::isSessionReadyToRun()
{
    if (m_session) {
...
void ConnectionImpl::runSession(LogRedirect &redirect)
{
